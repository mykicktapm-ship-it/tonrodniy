...
## Внесённые изменения
- Добавлена сервисная аутентификация для создания и финализации лобби, а также проверка пользовательских токенов для операций join/pay, чтобы закрыть доступ к Supabase-сервисным ключам из публичных запросов.
- В вебхуке TON включена проверка HMAC-подписи тела, allowlist IP-источников и idempotency по хешу события, чтобы исключить повторные или поддельные вызовы.
- Платёжные события теперь сверяются со ставкой лобби; любые несовпадения фиксируются в audit/tx логах и приводят к отклонению оплаты.
